...
DEFAULT_MODEL_PATH = "models/Qwen3-30B-A3B-Instruct-2507"
VLLM_HOST = "0.0.0.0"
VLLM_PORT = 8000
...
parser = argparse.ArgumentParser(description="Video-SALMONN2 local evaluation via vLLM.")
parser.add_argument("res_file", help="Caption JSON file")
...
def gpt_extract(item):
    try:
        ...
        print(f"[ERROR] Failed to process item {video_id}: {type(e).__name__}: {e}")
...
def gpt_extract_2(item):
    try:
        ...
        assert miss + cor + incor == len(item["events"]), \
            f"Sum mismatch: {miss} + {cor} + {incor} != {len(item['events'])}"
        ...
        video_id = item.get('id', 'unknown')
        print(f"[ERROR] Failed to process item (v2) {video_id}: {type(e).__name__}: {e}")
        print(f"[DEBUG] Traceback:\n{traceback.format_exc()}")
        return item
...
if __name__ == "__main__":
    ...
    output_json = res_file.replace(".json", "_eval_results_gpt41.json")
    eval_model_name = Path(MODEL_PATH).name
    log_path = str(res_file_path.parent / f"{model_name}_results_{eval_model_name}.log")
    ...
    print(f"[INFO] Total samples to process: {len(res_data)}")

    total_result = []

    for i in range(7):
        print(f"\n[INFO] ===== Round {i+1}/7 =====")
        with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:  # Reduced from 100 to 20
            responses = list(tqdm(executor.map(gpt_extract, res_data), total=len(res_data), desc=f"Round {i+1}"))
        ...
        print(f"[INFO] Output file: {output_json}")
        print(f"[INFO] Success: {len(result)}, Failed: {len(ignore)}")

        k = 0

        while len(ignore) != 0 and k < 8:
            print(f"[INFO] Retry round {k+1}/8 for {len(ignore)} failed items")
            with concurrent.futures.ThreadPoolExecutor(max_workers=min(20, len(ignore))) as executor:  # Reduced from 100 to 20
                responses = list(tqdm(executor.map(gpt_extract, ignore), total=len(ignore), desc=f"Retry {k+1}"))
        ...
        if len(ignore) > 0:
            k = 0
            print(f"[INFO] Switching to Version 2 GPT request for {len(ignore)} remaining items")
            while len(ignore) != 0 and k < 3:
                print(f"[INFO] Version 2 retry {k+1}/3")
                with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                    responses = list(tqdm(executor.map(gpt_extract_2, ignore), total=len(ignore), desc=f"V2 Retry {k+1}"))
